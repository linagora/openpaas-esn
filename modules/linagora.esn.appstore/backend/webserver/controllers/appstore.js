...
module.exports = function(appstoremanager) {
  ...
  function list(req, res) {
    ...
    appstoremanager.get(query, function(err, result) {
      if (err) {
        return res.status(500).json({ error: { code: 500, message: 'app list failed', details: err.message}});
      }
  ...
  function submit(req, res) {
    ...
    if (!application) {
      return res.status(400).json({error: { code: 400, message: 'Body missing', details: 'body is missing'}});
    }

    appstoremanager.store(application, function(err, saved) {
      if (err) {
        return res.status(500).json({ error: { code: 500, message: 'Cannot sumbit the application', details: err.message}});
      }

      return res.status(200).json({_id: saved._id});
    });
  }

  function deleteApp(req, res) {
    return res.status(500).json({ error: { code: 500, message: 'App delete failed', details: 'Not implemented yet'}});
  }
  ...
  function deploy(req, res) {
    ...
    appstoremanager.deploy(req.application, deployData, function(err) {
      if (err) {
        return res.status(500).json({ error: { code: 500, message: 'App save error', details: err.message}});
      }
  ...
  function updeploy(req, res) {
    return res.status(500).json({ error: { code: 500, message: 'App updeploy failed', details: 'Not implemented yet'}});
  }
  ...
  function undeploy(req, res) {
    ...
    appstoremanager.undeploy(req.application, target, function(err) {
      if (err) {
        return res.status(500).json({ error: { code: 500, message: 'App delete failed', details: err.message}});
      }
  ...
  function uploadAvatar(req, res) {
    if (!req.query.mimetype) {
      return res.status(400).json({error: { code: 400, message: 'Parameter missing', details: 'mimetype parameter is required'}});
    }

    var mimetype = req.query.mimetype.toLowerCase();
    if (acceptedImageTypes.indexOf(mimetype) < 0) {
      return res.status(400).json({error: { code: 400, message: 'Bad parameter', details: 'mimetype ' + req.query.mimetype + ' is not acceptable'}});
    }

    if (!req.query.size) {
      return res.status(400).json({error: { code: 400, message: 'Parameter missing', details: 'size parameter is required'}});
    }

    var size = parseInt(req.query.size, 10);
    if (isNaN(size)) {
      return res.status(400).json({error: { code: 400, message: 'Bad parameter', details: 'size parameter should be an integer'}});
    }
    ...
    function updateApplicationAvatar() {
      appstoremanager.updateAvatar(req.application, avatarId, function(err) {
        if (err) {
          return res.status(500).json({error: { code: 500, message: 'Datastore failure', details: err.message}});
        }
        return res.status(201).json({_id: avatarId});
      });
    }
    ...
    function avatarRecordResponse(err, storedBytes) {
      if (err) {
        if (err.code === 1) {
          ...
          return res.status(500).json({error: { code: 500, message: 'Image processing failure', details: err.message}});
        } else {
          return res.status(500).json({error: { code: 500, message: 'Internal server error', details: err.message}});
        }
  ...
  function getArtifact(req, res) {
    ...
    if (!artifactId) {
      res.status(400).json({error: { code: 400, message: 'Bad parameter', details: 'missing parameter artifactId'}});
    }

    var artifacts = req.application.artifacts;
    if (!artifacts.length) {
      return res.status(404).json({ error: { code: 404, message: 'Not found', details: 'artifact not found'}});
    }
    ...
    appstoremanager.getArtifact(artifactId, function(err, readable) {
      if (err) {
        return res.status(500).json({ error: { code: 500, message: 'Cannot get artifact', details: err.message}});
      }
  ...
  function uploadArtifact(req, res) {
    ...
    if (!req.query.version) {
      return res.status(400).json({error: { code: 400, message: 'Parameter missing', details: 'version parameter is required'}});
    }
    ...
    var saveArtifact = function(stream) {
      appstoremanager.uploadArtifact(req.application, mimetype, metadata, stream, options, function(err, fileMetadata) {
        if (err) {
          return res.status(500).json({ error: { code: 500, message: 'Cannot upload artifact', details: err.message}});
        }
        ...
        appstoremanager.updateArtifact(req.application, artifactMetadata, function(err) {
          ...
          return res.status(201).json({_id: fileMetadata._id});
        });
      });
    };
  ...
  function install(req, res) {
    ...
    appstoremanager.install(req.application, target, function(err) {
      if (err) {
        return res.status(500).json({ error: { code: 500, message: 'Could not install app', details: err.message}});
      }
  ...
  function uninstall(req, res) {
    ...
    appstoremanager.uninstall(req.application, target, function(err) {
      if (err) {
        return res.status(500).json({ error: { code: 500, message: 'Could not uninstall app', details: err.message}});
      }
