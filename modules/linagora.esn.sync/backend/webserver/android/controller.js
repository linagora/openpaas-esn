...
module.exports = dependencies => {
  ...
  function androidGuideFile(locale) {
    return path.normalize(path.join(__dirname, '../../lib/i18n/guides/android/' + locale + '.md'));
  }
  ...
  function fallbackToEnglishGuide(locale) {
    return err => {
      logger.warn(`Could not read android guide from the filesystem using locale ${locale}. Falling back to english version`, err);
