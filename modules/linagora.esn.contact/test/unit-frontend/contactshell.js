...
describe('ContactShell factory', function() {
  var ICAL, $window, notificationFactory;
  ...
  function buildFakeVcard() {
    ...
    for (var i = 0, types = ['Home', 'Work', 'Other'], socials = ['Skype', 'Twitter', 'Other']; i < types.length; i++) {
  ...
  function checkShellContactCreatedObject(shell, etag) {
    ...
    expect(shell.tags.map(function(tag) { return tag.text; })).to.deep.equal(['tag1', 'tag2', 'tagn']);
    ...
    for (var i = 0, types = ['Home', 'Work', 'Other'], socials = ['Skype', 'Twitter', 'Other']; i < types.length; i++) {
      ...
      expect(shell.addresses[i]).to.deep.equal({type: types[i],
