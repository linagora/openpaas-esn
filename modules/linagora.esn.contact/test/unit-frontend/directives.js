  ...
  describe('inlineEditableInput', function() {
    ...
    it('should have the proper template', function() {
      ...
      expect(element.html()).to.deep.equal(
        '<div class="input-group"><input type="aType" placeholder="aPlaceholder" ng-model="ngModel" ng-model-options="{updateOn: &quot;blur&quot;}" class="aClass"><span ng-show="showGroupButtons" class="input-group-btn hidden-xs ng-hide"><button type="button" ng-click="resetInput()" class="btn btn-default"><i class="mdi mdi-close"></i></button></span><!-- ngIf: lastModifiedFlag && updateSuccessFlag --><span ng-show="showGroupButtons" class="input-group-btn hidden-xs ng-hide"><button type="button" class="btn btn-default"><i class="mdi mdi-check"></i></button></span></div>');
    });
