...
.factory('registerTimezones', function(ICAL, TIMEZONES) {
  function buildTimezone(tzid, ics) {
    return ICAL.TimezoneService.get(tzid) || new ICAL.Timezone(new ICAL.Component(ICAL.parse(ics)));
  }

  function findTimezone(tzid) {
    if (tzid === 'UTC') {
      return ICAL.Timezone.utcTimezone;
    } else if (TIMEZONES.zones[tzid]) {
      return buildTimezone(tzid, TIMEZONES.zones[tzid]);
    } else {
      return findTimezone(TIMEZONES.aliases[tzid].aliasTo);
    }
  }
