...
Jcal is a JSON equivalent of Ical
(we use http://mozilla-comm.github.io/ical.js/ for parsing/generating ICal & Jcal from/to js objects)

## Backend

### Sabre

SabreDAV is a Php server implementing the WebDAV, CalDAV & CardDAV standards.

The calendar module part is using the CalDAV part of Sabre.
...
All data of sabre is stored in Mongo. For that we developed a specific class to handle all necessary operations on calendar objects : lib/CalDAV/Backend/Mongo.php
...
To do this it queries directly the ESN MongoDB and builds the principals out of the objects stored there. (cf lib/DAVACL/PrincipalBackend/Mongo.php)
...
This file is built as a Sabre Plugin (cf http://sabre.io/dav/writing-plugins/) and it defines routes designed for our needs.

#### Scheduling

Scheduling (https://tools.ietf.org/html/rfc6638) is a feature defined aside CalDav and implemented by Sabre which allows to manage the workflow when a user is invited to an event.

All the workflow of invitations is handled by Sabre and events are generated every time a user needs to be notified due to an invitation.

We defined a file (`lib/CalDAV/Schedule/IMipPlugin.php`) which handles theses events and calls a specific route (`ESN/api/calendar/calendars/inviteAttendee`) in the ESN backend to send appropriate emails.
...
All changes done on calendars objects in Sabre can be done by a different clients: the ESN frontend or any Caldav client...

So they have to be propagated to the ESN to have our frontend display of events consistent with the actual state of the events in Sabre.

If a user modifies an event form ThunderBird, we want to have all its open OP clients to show this modification in realtime.

To achieve that, we developed a Sabre plugin which pushes a message in RabbitMQ each time something happens which concerns a calendar/event.

The ESN then listens to the RabbitMQ and propagates the action from Sabre to our OP clients using websockets.
...
This file defines most of the behavior of interactions between our frontend code and the calendar display.
...
* calendar-api and event-api defines all operations needed by our clients using the request service and uses Jcal objects.
* calendar-service and event-service cale the *-apis method and wrap params and method returns in shells.
...
In OP search is done using ElasticSearch.
Each module wanting to be searchable has to provide 3 things:
...
The backend part interacting with ES is in `modules/linagora.esn.calendar/backend/lib/alarm/index.js`
The client side (provider) is : `modules/linagora.esn.calendar/frontend/app/services/events-provider/events-provider.js`
