...
describe('The synchronize worker', function() {
  ...
  const { SYNCHRONIZE } = require('../../../../backend/lib/constants').JOBQUEUE_WORKER_NAMES;

  beforeEach(function() {
    mockery.registerMock('../helper', () => ({}));

    getModule = () => require('../../../../backend/lib/workers/synchronize')(this.moduleHelpers.dependencies);
  });

  it('should return correct worker name', function() {
    expect(getModule().name).to.equal(SYNCHRONIZE);
  });
  ...
  describe('The handle method', function() {
    ...
    it('should reject if the failed to clean out dated contacts', function(done) {
      const job = {
        data: {
          user: { bar: 'bar' },
          account: {
      ...
      importerRegistryMock.get = () => ({ lib: { importer: { importContact: () => Promise.resolve() } } });
      helperMock.getImporterOptions = () => Promise.resolve();
      helperMock.initializeAddressBook = () => Promise.resolve(options);
      helperMock.cleanOutdatedContacts = sinon.stub().returns(Promise.reject(new Error('something wrong')));

      getHandleMethod()(job)
        .then(() => done('should not resolve'))
        .catch(err => {
          expect(helperMock.cleanOutdatedContacts).to.have.been.calledOnce;
          expect(err.message).to.equal('something wrong');
          done();
        });
    });
    ...
    it('should resolve if the success to synchronize contacts', function(done) {
      ...
      helperMock.getImporterOptions = () => Promise.resolve();
      helperMock.initializeAddressBook = () => Promise.resolve(options);
      helperMock.cleanOutdatedContacts = sinon.stub().returns(Promise.resolve());
  ...
  describe('The getTitle method', function() {
    it('should build the correct title', function() {
      ...
      expect(getModule().handler.getTitle(jobData)).to.equal(`Synchronize ${jobData.account.data.provider} contacts for user ${jobData.user._id}`);
    });
  });
});
