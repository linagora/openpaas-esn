...
describe('PUT api/users/:uuid/emails route', function() {
  ...
  describe('when the allowing domain admin to manage user emails is disabled', function() {
    beforeEach(function(done) {
      core['esn-config']('allowDomainAdminToManageUserEmails')
        .inModule('core')
        .store(false)
        .then(() => done())
        .catch(done);
    ...
    it('should respond 403 when domain admin tries to update user emails', function(done) {
  ...
  describe('when the allowing domain admin to manage user emails is enabled', function() {
    beforeEach(function(done) {
      core['esn-config']('allowDomainAdminToManageUserEmails')
        .inModule('core')
        .store(true)
