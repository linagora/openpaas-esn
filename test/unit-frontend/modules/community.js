...
describe('The Community Angular module', function() {
  ...
  describe('the communityInjectionsWidget directive', function() {
    ...
    beforeEach(angular.mock.inject(function($rootScope, $compile) {
      ...
      this.html = '<div community-injections-widget=' + this.anchorId + '/>';
    }));

    it('should hide the element if no anchor is defined', function() {
      var noAnchorHtml = '<div community-injections-widget />';
      var element = this.$compile(noAnchorHtml)(this.scope);
      this.scope.$digest();
      expect(element[0].getAttribute('style')).to.contain('display: none');
    });

    it('should hide the element if community has no injections', function() {
      var element = this.$compile(this.html)(this.scope);
      this.scope.$digest();
      expect(element[0].getAttribute('style')).to.contain('display: none');
    });

    it('should hide the element if community has no injections for this anchor', function() {
      this.scope.community.injections = [
        {
          key: 'otherAnchor',
          values: [
